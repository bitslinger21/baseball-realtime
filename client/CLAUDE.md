...
Three items previously listed as open have been confirmed done against the actual code:

1. **PR 6.6 + BUG-011 lean Pitching tab — DONE.** `PitchingTab` is per-player and renders real data: Performance by pitch type (from `pitchLog` stat type, server-side aggregated) + By pitcher handedness (from splits API). No shared Peña mock. BUG-011 resolved via option 2 ("redesign down"); rich five-card version parked as `PitchingTabFull` pending Statcast.
2. **BUG-008 — FIXED.** The LIVE pill is gated on `isLive`; a final game shows no pill. Fixed as part of PR 11.
3. **F-003 — DONE (PR 13).** `ScorebookCell` now draws both strokes: bold path = PA result at the plate, lighter path = later baserunning. Ships now; only F-006 (FC / spray / error attribution) stays parked.
...
- **At-bats scorebook row** (in the batter card) — each of today's PAs as a scorebook diamond cell (`ScorebookCell` atom); the "Today" line shows only the summary, the per-AB results live in the diamonds. Live PA is neutral, not rust. F-003 two-stroke enhancement (bold PA result vs. lighter later-baserunning) shipped in PR 13 (Jun 21, 2026) — `ScorebookCell` draws both; F-006 (FC/spray/error attribution) stays parked.
...
- **Scout mode (game view, finals)** — **DESIGNED Jun 20, 2026**, recorded in `future.md` **F-007**; handoff in `PROMPT_F007_scout_mode.md` + `Game Scout Mode.html` prototype. One play head; **Play/Pause toggles Replay↔Scout** (Replay = pitches auto-advance, Scout = paused/analyze); finals open in Scout paused at game start; clicking a **pitch-feed PA** or a **batter-card scorebook cell** seeks the head to that AB's end; the head is a **past/future boundary** on both the feed and the scorebook row. No new API — reuses the existing feed + atoms. Ungated, pending sign-off + port.
