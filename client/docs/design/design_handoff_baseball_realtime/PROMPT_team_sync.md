...
### Team leaders (§8) — absent entirely

Not stubbed, not rendered, no placeholder: the card is simply not in `TeamPage.tsx`. Design
specifies two stacked top-3 lists — **Home runs** then **Batting average** — split by a `borderLight`
divider, each row `rank · name (→ player page) · value`, with a `Bat / Pitch` Segmented (Bat only for
now).

The original prompt flagged the data as unconfirmed ("may be `/leaders` filtered by team"). It can be:
the leaders service already fetches full leaderboards, and `StandingTeamDto` gives the roster's team
identity — but note the **`limit=10` scoping bug** in `handoff_leaders_sync/PROMPT_leaders_sync.md`
BUG 2 applies here too. A team's HR leader will frequently not be in the MLB top 10, so filtering the
existing payload by team yields empty cards for most teams. This card needs a **team-scoped query**,
not a filter.

### Roster (§6) — stub

Renders `Roster data coming soon`. Design specifies a **table** (not cards) grouped by position with
`surfaceAlt` group rows (`Infield` / `Outfield` / `Catcher`), columns `# · Name · Pos · AVG · HR ·
RBI · OPS`, numerals mono and right-aligned, name links to `/player/:mlbId`, row hover `surfaceAlt`.
The `Batters / Pitchers` Segmented is correctly present and inert.

---

## Empty data in shipped cards

| Field | Shipped | Design |
|---|---|---|
| Recent form splits — Home / Away / 1-Run | all three render `—` | real records |
| Hero founded year | `Est. —` | `Est. 1962` |
| Hero venue + city | absent | `Daikin Park · Houston, TX` |

The splits were flagged "source unconfirmed" in the original prompt and are still unconfirmed —
`StandingTeamDto` has no home/away/one-run breakdown. MLB's standings endpoint does return
`records.splitRecords` (home, away, oneRun among them); if that is already in the raw payload the lift
is a type → mapper → wire addition, the same shape as the PR 3.5 win-prob work.

Venue and founded year are on the MLB teams endpoint, which `api/src/teams/teams-meta.service.ts`
already exists to serve. Worth checking whether it's simply unwired on this page.

**Decision needed:** wire these three, or sync the design down to omit them? Same question as the
player view's Contact quality rows — and the answer there was wire.
