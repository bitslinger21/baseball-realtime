  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr', gap: 20 }}>
        {/* Logo */}
        <Card title="Logo" subtitle="The Scorebook — rust diamond mark">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <img src="assets/logo-rust-on-light.png" alt="Logo on light" style={{ width: '100%', borderRadius: T.r.sm, border: `1px solid ${T.border}` }} />
            <img src="assets/logo-rust-on-dark.png" alt="Logo on dark" style={{ width: '100%', borderRadius: T.r.sm, border: `1px solid ${T.border}` }} />
          </div>
        </Card>
      ...
      <Card title="Game-state primitives" subtitle="Bases, count, inning, strike zone, team marks" style={{ marginTop: 20 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'auto auto auto auto 1fr', gap: 28, alignItems: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
            <Bases on={[true, true, false]} runners={['S. Suzuki', 'I. Happ', null]} size={42} />
            <Eyebrow>Bases · hover</Eyebrow>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
      ...
      {/* Scorebook at-bats */}
      <Card title="Scorebook at-bats" subtitle="Bold = what he did at the plate · light = baserunning after · green = run · hollow ring = left on base · × = out on the bases" style={{ marginTop: 20 }}>
        <div style={{ display: 'flex', gap: 18, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          {[
            ['Single', { inn: '1st', code: '1B', kind: 'hit',  reachedOnPA: 1, finalBase: 1 }],
            ['Double', { inn: '2nd', code: '2B', kind: 'hit',  reachedOnPA: 2, finalBase: 2 }],
            ['Walk',   { inn: '3rd', code: 'BB', kind: 'walk', reachedOnPA: 1, finalBase: 1 }],
            ['Hit by pitch', { inn: '3rd', code: 'HBP', kind: 'hbp', reachedOnPA: 1, finalBase: 1 }],
            ['Singled · scored', { inn: '4th', code: '1B', kind: 'hit', reachedOnPA: 1, finalBase: 4, scored: true }],
            ['Walked · stranded', { inn: '5th', code: 'BB', kind: 'walk', reachedOnPA: 1, finalBase: 2, stranded: true }],
            ['Caught stealing', { inn: '6th', code: '1B', kind: 'hit', reachedOnPA: 1, outAt: 2 }],
            ['Strikeout', { inn: '7th', code: 'K',  kind: 'out',  reached: 0 }],
            ['Groundout', { inn: '8th', code: '6-3', kind: 'out', reached: 0 }],
            ['Out · generic', { inn: '8th', code: 'OUT', kind: 'out', reached: 0 }],
            ['Home run', { inn: '9th', code: 'HR', kind: 'hit', reached: 4 }],
            ['Live PA', { inn: '9th', code: '1-1', live: true }],
          ].map(([label, props]) => (
