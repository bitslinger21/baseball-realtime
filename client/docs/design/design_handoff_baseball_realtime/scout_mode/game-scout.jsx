...
// One marker; the whole game-v2 screen reflects it.
...
const AUTO = {
  '1B': 'Single', '2B': 'Double', '3B': 'Triple', 'HR': 'Home run', 'BB': 'Walk', 'IBB': 'Intentional walk', 'K': 'Strikeout', 'HBP': 'Hit by pitch',
...
const SCRIPT = [
  ...
  { inn: 6, half: 'top', abs: [['paredes', '1B', 'hit', { r: 1, f: 1 }], ['meyers', 'IBB', 'walk', { r: 1, f: 1, t: 'Intentional walk' }], ['vazquez', '6-4-3', 'out', { t: 'Double play' }]] },
...
// Slots hold the RUNNER'S bid (or null) rather than a boolean, so the Bases atom's
// hover can name who is on — occupancy is just Boolean(slot).
function basesAt(H) {
  const cur = MOMENTS[H].ab;
  const order = GAME.filter(a => a.inn === cur.inn && a.half === cur.half);
  const b = [null, null, null]; // 1B, 2B, 3B
  for (const a of order) {
    if (a.idx >= cur.idx) break;
    const r = a.sb.reachedOnPA != null ? a.sb.reachedOnPA : 0;
    if (a.sb.kind === 'out') continue;
    if (r >= 4) { b[0] = b[1] = b[2] = null; continue; }
    if (a.sb.kind === 'walk' || a.sb.kind === 'hbp') {
      if (b[0]) { if (b[1]) { b[2] = b[1]; } b[1] = b[0]; }
      b[0] = a.bid;
    } else if (r === 1) {
      b[2] = b[1]; b[1] = b[0]; b[0] = a.bid;
    } else if (r === 2) {
      b[2] = b[0]; b[1] = a.bid; b[0] = null;
    } else if (r === 3) {
      b[0] = null; b[1] = null; b[2] = a.bid;
    }
  }
  return b;
}
...
// ============================================================
// SCOUT BAND — aligned with the live view (Sep 6, 2026). It used to be its own
// three-zone 164px block (line score · game leaders · last pitch). It is now a thin
// wrapper around the SAME window.LineScoreBand the live view uses: 48px sticky bar
// carrying the score as of the marker, plus an overlay drawer with the inning grid
// and game leaders. Everything stays head-driven — the wrapper just derives arrays.
//
// The last-pitch zone is CUT here for the same reason it was cut from the live band:
// the pitch-by-pitch feed shows the pitch the user just watched, in the spot the user
// is already looking. (This reverses the Jul 18 pass that moved it INTO the band.)
function ScoutBand({ H, curAB }) {
  ...
  // Per-inning arrays through the marker: an inning the play head has not reached
  // yet is null, which the shared band renders as a dash.
  const innCount = Math.max(9, curAB.inn);
  const seriesFor = (team, half) => Array.from({ length: innCount }, (_, i) =>
    reached(i + 1, half) ? inningRuns(team, i + 1, half) : null);

  // Best day per team through the marker, mapped into the band's leader shape.
  ...
  // Two slots always, so the drawer never changes height when a second leader
  // appears; the shared band renders a null slot as reserved space.
  const leaders = ['HOU', 'CHC'].map(t => {
    const best = leaderFor(t);
    if (!best) return null;
    const b = BATTERS[best.bid];
    return { team: TEAMS[b.team], name: b.name, line: leaderLine(best.d) };
  });

  return (
    <window.LineScoreBand
      curInning={curAB.inn}
      runsAway={seriesFor('HOU', 'top')}
      runsHome={seriesFor('CHC', 'bot')}
      totals={{
        away: [teamRuns('HOU', H), teamHits('HOU', H), teamErrors('HOU', H)],
        home: [teamRuns('CHC', H), teamHits('CHC', H), teamErrors('CHC', H)],
      }}
      leaders={leaders}
      // Silence state: the insight generator is not marker-aware, and a mid-game
      // candidate array shown at a finished/rewound marker contradicts the screen
      // (it named a pitcher who is not even in this dataset). The ask affordance
      // alone is honest here until the generator derives from the marker.
      iqCandidates={[]}
      iqSuggested={[]}
    />
  );
    ...
    <Card padless>
      ...
      <div style={{ padding: '11px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${T.border}`, background: T.surfaceAlt }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
          ...
          <Bases on={bases.map(Boolean)} runners={bases.map(id => id && BATTERS[id].name)} size={26} fill={T.accent} empty={T.borderStrong} strokeWidth={2} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 0 }}>
        ...
        <div style={{ padding: 18, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Eyebrow style={{ fontSize: 9 }}>{midAB ? 'At bat' : 'At the marker'} · {b.team}</Eyebrow>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr)', gap: 6, marginTop: 4 }}>
            <div style={{ padding: '8px 10px', border: `1px solid ${T.border}`, borderRadius: T.r.sm, display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 8 }}>
              <span style={{ fontSize: 10, color: T.textMuted, fontFamily: T.sans, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Today · through marker</span>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: 9.5, color: T.textFaint, fontFamily: T.sans, fontWeight: 500, whiteSpace: 'nowrap' }}>tap to move the marker</span>
              </div>
              ...
              <div style={{ marginTop: 5, fontSize: 9.5, color: T.textFaint, fontFamily: T.sans, display: 'flex', alignItems: 'center', gap: 5 }}>
                <span style={{ width: 18, height: 1, background: T.borderStrong }} /> greyed = not at the marker
              </div>
            </div>
          </div>
        </div>
...
function MatchupContext({ H, onSeek }) {
  ...
  return (
    <Card padless>
      {/* On the mound — condensed from the old full-width MoundCard (Aug 26, 2026),
          matching game-v2: heads this card because the head-to-head below is
          "batter vs THIS pitcher". Head-aware, so it re-reads as the marker moves. */}
      {(() => {
        const pt = MOMENTS[H].ab.team === 'HOU' ? 'CHC' : 'HOU';
        const p = PITCHERS[pt];
        return (
          <div style={{ padding: '9px 16px', borderBottom: `1px solid ${T.border}`, background: T.surfaceAlt, display: 'grid', gridTemplateColumns: 'auto minmax(0,1fr) auto', gap: 10, alignItems: 'center' }}>
            <Headshot team={TEAMS[pt]} initials={p.last.slice(0, 2).toUpperCase()} mlbId={p.mlb} size={30} ratio={1.15} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 1, minWidth: 0 }}>
              <Eyebrow style={{ fontSize: 11 }}>Pitching · {pt}</Eyebrow>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, minWidth: 0 }}>
                <span style={{ fontFamily: T.sans, fontSize: 14, fontWeight: 700, letterSpacing: '-0.01em', whiteSpace: 'nowrap' }}>{p.name}</span>
                <span style={{ fontFamily: T.mono, fontSize: 11, color: T.textFaint, whiteSpace: 'nowrap' }}>{p.hand} · #{p.num}</span>
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: 'flex-end', fontFamily: T.mono, fontSize: 12.5, fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                {/* ipString() already appends " IP"; the unit is the faint label here */}
                <span style={{ fontWeight: 700, color: T.text }}>{ipString(pitcherOuts(pt, H)).replace(' IP', '')} <span style={{ fontSize: 11, fontWeight: 500, color: T.textFaint }}>IP</span></span>
                <span style={{ color: T.textMuted }}>{pitcherLineThrough(pt, H)}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, color: T.textMuted, fontSize: 11.5 }}>
                <span>{p.pitches} <span style={{ color: T.textFaint }}>P</span></span>
                <span style={{ color: T.textFaint }}>·</span>
                <span><span style={{ color: T.textFaint }}>ERA</span> {p.era}</span>
                <span style={{ color: T.textFaint }}>·</span>
                <span><span style={{ color: T.textFaint }}>WHIP</span> {p.whip}</span>
              </div>
            </div>
          </div>
        );
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
        ...
        <div style={{ padding: '13px 16px 15px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Eyebrow>Due up · after the marker</Eyebrow>
    ...
    <Card padless>
      ...
      <div style={{ padding: 20, display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 20, alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <Eyebrow style={{ fontSize: 11 }}>Pitching · {pitchTeam}</Eyebrow>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          {[{ label: 'Today', value: today, sub: todaySub }, { label: 'Pitches', value: p.pitches, sub: p.strikes }, { label: 'ERA', value: p.era, sub: 'season' }, { label: 'WHIP', value: p.whip, sub: 'season' }].map(s => (
            <div key={s.label} style={{ padding: '10px 14px', border: `1px solid ${T.border}`, borderRadius: T.r.sm, minWidth: 110, display: 'flex', flexDirection: 'column', gap: 2, background: T.surface }}>
              ...
              <span style={{ fontFamily: T.mono, fontSize: 11, color: T.textFaint }}>{s.sub}</span>
            </div>
          ))}
        </div>
      </div>
...
function ScoutFeed({ H, onSeek, onStep, onStepPitch, feedRef, rowRefs, playing, togglePlay, N, speed, setSpeed }) {
  ...
  // The marker defines the "current" AB — pinned as a canvas in the middle.
  ...
  // Scroll-driven marker: the three zones are one chronological filmstrip.
  ...
  return (
    <div ref={rootRef} style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.r.lg, boxShadow: T.sh.sm, display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, overflow: 'hidden' }}>
      <div style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, borderBottom: `1px solid ${T.border}`, flexShrink: 0, minWidth: 0 }}>
        <span style={{ fontFamily: T.sans, fontSize: 15, fontWeight: 700, color: T.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Pitch by pitch</span>
        <span style={{ fontFamily: T.mono, fontSize: 11, color: T.textMuted, flexShrink: 0 }}>{H + 1}/{N}</span>
      </div>

      {/* Timeline — occupies the space the Upcoming zone used to hold, directly
          above the pinned at-bat. */}
      <div style={{ flexShrink: 0, borderBottom: `1px solid ${T.border}`, background: T.surface }}>
        <GameTimeline H={H} onSeek={onSeek} awayAbbr="HOU" homeAbbr="CHC" />
      </div>

      {/* Current-AB canvas — marker AB pinned in the middle, pitches scroll below.
      ...
      {curAB && (
        <div key={curAB.idx} className="ab-canvas-enter" style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: T.surfaceAlt + '55', borderLeft: `3px solid ${T.ink}` }}>
          <div ref={el => { rowRefs.current[curAB.idx] = el; }} className="ab-header-enter" style={{ flexShrink: 0, display: 'grid', gridTemplateColumns: '74px 32px 1fr auto', gap: 12, alignItems: 'center', padding: '12px 16px', background: T.surfaceAlt + 'cc', borderBottom: `1px solid ${T.border}` }}>
            ...
            {/* Transport lives on the batter's row, right-justified — the controls sit
                where attention already is, and the row is pinned so they never scroll away. */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}>
              <select value={speed} onChange={e => setSpeed(Number(e.target.value))} title="Playback speed"
                style={{ flexShrink: 0, fontFamily: T.mono, fontSize: 11, fontWeight: 700, color: T.text, background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.r.pill, padding: '5px 4px', cursor: 'pointer', width: 44 }}>
                {[0.5, 1, 2, 4].map(s => <option key={s} value={s}>{s}×</option>)}
              </select>
              <button onClick={() => onSeek(0)} title="Replay from first pitch" style={{ ...stepBtn, width: 28, height: 28, flexShrink: 0, background: T.surface, fontSize: 11 }}>⟲</button>
              <button onClick={() => onStep(-1)} title="Previous at-bat" style={{ ...stepBtn, width: 28, height: 28, flexShrink: 0, background: T.surface }}>⏮</button>
              <button onClick={togglePlay} title={playing ? 'Pause' : 'Play'}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, height: 28, padding: '0 12px', background: 'transparent', color: playing ? T.accent : T.text, border: `1.5px solid ${playing ? T.accent : T.borderStrong}`, borderRadius: T.r.pill, cursor: 'pointer', flexShrink: 0, fontSize: 11, fontFamily: T.sans, fontWeight: 700 }}>
                <span style={{ fontSize: 10 }}>{playing ? '⏸' : '▶'}</span>{playing ? 'Pause' : 'Play'}
              </button>
              <button onClick={() => onStep(1)} title="Next at-bat" style={{ ...stepBtn, width: 28, height: 28, flexShrink: 0, background: T.surface }}>⏭</button>
            </div>
          </div>
...
// ============================================================
// GameTimeline — the whole game as one rail. Segments are HALF-INNINGS whose
// widths are proportional to pitches thrown, so a grinding inning is visibly
// wider than a quick one (an out-per-stop rail would make them equal and hide
// exactly the thing worth seeing). Two run lanes — away above the rail, home
// below — each labelled with its team logo so lane position, not colour alone,
// carries the identity; the colours (navy away, rust home) are a second cue.
// Drag = pitch resolution; release snaps to the nearest at-bat boundary so you
// always land somewhere meaningful rather than mid-count.
// ============================================================
function GameTimeline({ H, onSeek, awayAbbr, homeAbbr }) {
  const railRef = React.useRef(null);
  const dragging = React.useRef(false);
  const [hoverPct, setHoverPct] = React.useState(null);

  // half-inning segments, in play order
  const segments = React.useMemo(() => {
    const out = [];
    MOMENTS.forEach((mo, i) => {
      const key = `${mo.ab.inn}-${mo.ab.half}`;
      const cur = out[out.length - 1];
      if (cur && cur.key === key) cur.count++;
      else out.push({ key, inn: mo.ab.inn, half: mo.ab.half, start: i, count: 1 });
    });
    return out;
  }, []);

  const runs = React.useMemo(() => GAME.filter(a => a.runs > 0).map(a => ({
    at: a.last, pct: (a.last / (N - 1)) * 100, team: a.team, n: a.runs, label: `${a.inning} · ${a.result}`,
  })), []);

  const pctFor = i => (i / (N - 1)) * 100;
  ...
  const snap = (idx) => {
    let best = GAME[0].last, d = Infinity;
    for (const a of GAME) { const dd = Math.abs(a.last - idx); if (dd < d) { d = dd; best = a.last; } }
    return best;
  };
  ...
  const onDown = (e) => {
    ...
    onSeek(idxFromEvent(e));
    const move = (ev) => { if (dragging.current) onSeek(idxFromEvent(ev)); };
    const up = (ev) => {
      dragging.current = false;
      onSeek(snap(idxFromEvent(ev)));
  ...
  const lane = (which) => (
    <div style={{ position: 'relative', height: 11 }}>
      {runs.filter(r => (which === 'away' ? r.team === awayAbbr : r.team === homeAbbr)).map((r, i) => (
        <span key={i} title={`${r.n} run${r.n > 1 ? 's' : ''} · ${r.label}`} style={{
          ...
          background: which === 'away' ? T.info : T.accent,
          top: which === 'away' ? 'auto' : 1, bottom: which === 'away' ? 1 : 'auto',
        }} />
      ))}
    </div>
  );

  const markerPct = pctFor(H);
  return (
    <React.Fragment>
      <div style={{ padding: '10px 16px 12px', display: 'grid', gridTemplateColumns: '26px 1fr', columnGap: 10, alignItems: 'center' }}>
        {/* lane labels */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0, alignItems: 'center' }}>
          <span style={{ height: 11, display: 'grid', placeItems: 'center' }}><TeamDot team={TEAMS[awayAbbr]} size={13} /></span>
          <span style={{ height: 12 }} />
          <span style={{ height: 11, display: 'grid', placeItems: 'center' }}><TeamDot team={TEAMS[homeAbbr]} size={13} /></span>
        </div>
        <div>
          {lane('away')}
          {/* the rail */}
          <div ref={railRef} onPointerDown={onDown}
            onPointerMove={(e) => setHoverPct(((idxFromEvent(e)) / (N - 1)) * 100)}
            onPointerLeave={() => setHoverPct(null)}
            style={{ position: 'relative', height: 12, display: 'flex', gap: 1, cursor: 'pointer', touchAction: 'none', userSelect: 'none' }}>
            {segments.map((s, i) => (
              <div key={i} title={`${s.half === 'top' ? '▲' : '▼'}${s.inn} · ${s.count} pitches`}
                style={{ flex: s.count, background: s.half === 'top' ? T.surfaceAlt : T.border, borderRadius: 2 }} />
            ))}
            {hoverPct != null && (
              <span style={{ position: 'absolute', top: -2, bottom: -2, left: `${hoverPct}%`, width: 1, background: T.borderStrong, pointerEvents: 'none' }} />
            )}
            {/* marker */}
            {/* Marker owns gold — distinct from the navy/rust run pips, so position
                can never be misread as a score. */}
            <span style={{ position: 'absolute', top: -6, bottom: -6, left: `${markerPct}%`, width: 2, background: T.highlight, pointerEvents: 'none' }} />
            <span style={{ position: 'absolute', top: -11, left: `${markerPct}%`, transform: 'translateX(-50%)', width: 11, height: 11, borderRadius: '50%', background: T.highlight, border: `2px solid ${T.surface}`, boxSizing: 'content-box', pointerEvents: 'none' }} />
          </div>
          {lane('home')}
          {/* inning numbers, centred under each inning's two half-inning segments */}
          <div style={{ display: 'flex', gap: 1, marginTop: 3 }}>
            {(() => {
              const byInn = [];
              segments.forEach(s => {
                const cur = byInn[byInn.length - 1];
                if (cur && cur.inn === s.inn) cur.count += s.count;
                else byInn.push({ inn: s.inn, count: s.count });
              });
              return byInn.map((b, i) => (
                <div key={i} style={{ flex: b.count, textAlign: 'center', fontFamily: T.mono, fontSize: 10, color: T.textFaint, fontVariantNumeric: 'tabular-nums' }}>{b.inn}</div>
              ));
            })()}
          </div>
        </div>
...
window.GameScoutPrototype = function GameScoutPrototype({ showIntro = true } = {}) {
  // Finals open with the marker at the END of the game (the common case is "what
  // happened"); "Replay from first pitch" snaps it to zero.
  const [head, setHead] = React.useState(N - 1);
    ...
    <div style={{ maxWidth: 1340, margin: '0 auto', fontFamily: T.sans, color: T.text }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <p style={{ margin: 0, fontSize: 14, color: T.textMuted, lineHeight: 1.5, maxWidth: 900 }}>
          The same game view — one <b style={{ color: T.text }}>marker</b> the whole screen reflects: line score, count, the matchup, the batter card and the feed all read from it.
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {/* below the fold — mirrors game-v2. <MoundCard /> unmounted Aug 26, 2026;
            its content now heads MatchupContext above the fold. */}
