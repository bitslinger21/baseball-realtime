...
//   kind         'hit' | 'out' | 'walk'  (drives the basepath/dot styling)
...
window.ScorebookCell = function ScorebookCell({
  ...
  live = false, scored = false, width = 50,
}) {
  ...
  return (
    <div style={{
      flexShrink: 0, width,
      border: `1px ${live ? 'dashed' : 'solid'} ${live ? T.borderStrong : T.border}`,
      borderRadius: T.r.sm,
      background: T.surface,
      display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3,
      padding: '6px 3px 5px',
    }}>
      <span style={{ fontFamily: T.mono, fontSize: 9, fontWeight: 600, whiteSpace: 'nowrap', color: live ? T.textMuted : T.textFaint }}>{inn}</span>
      <svg width="36" height="36" viewBox="0 0 44 44" style={{ display: 'block' }}>
        ...
        <polygon points="22,41 41,22 22,3 3,22" fill="none" stroke={T.border} strokeWidth="1.5" />
        {/* later baserunning — light, drawn first so the bold PA stroke sits on top at the junction */}
        {lightD && (
          <path d={lightD} fill="none" stroke={T.textMuted} strokeWidth="1.5"
            strokeLinejoin="round" strokeLinecap="round" />
        )}
        {/* PA result — bold; dashed for a walk */}
        {boldD && (
          <path d={boldD} fill="none" stroke={T.ink} strokeWidth="3"
            strokeLinejoin="round" strokeLinecap="round"
            strokeDasharray={kind === 'walk' ? '3 3' : undefined} />
        )}
        {marker && marker.type === 'reach' && (
          <circle cx={pts[marker.idx][0]} cy={pts[marker.idx][1]} r="3.2"
            fill={kind === 'walk' ? T.surface : T.ink} stroke={T.ink} strokeWidth="1.5" />
        )}
        {marker && marker.type === 'lob' && (
          <circle cx={pts[marker.idx][0]} cy={pts[marker.idx][1]} r="3.4"
            fill={T.surface} stroke={T.textMuted} strokeWidth="1.5" />
        )}
      ...
      <span style={{
        ...
        color: live ? T.textMuted : (isOut ? T.textMuted : T.text),
      }}>{code}</span>
    </div>
  );
};
