# F-007 — Game review (finals) · design handoff

> Mirrors the `position_model/` pattern. One play head for a **final** game; the whole game view
> reflects it. **The single control toggles `▶ Review` (paused) ↔ `⏸ Play` (running).** Pure client —
> **no new API, no new data** (runs on the play-by-play history the game view already hydrates).
> Finals only (v1).

## Naming
- **Play** = playing (pitches auto-advance).
- **Review** = paused (head frozen; analyze).
- One control: action icon (▶/⏸) + the current-mode tag (Play / Review). Internal identifiers in the
  prototype may still read `scout*` — only the labels matter.

## Files
- **`../PROMPT_F007_scout_mode.md`** — the implementation prompt (self-contained spec).
- **`../PROMPT_F007_fixes.md`** — two follow-up fixes (position restore on return + selected-cell prominence).
- **`Game Scout Mode.html`** — runnable prototype (open in a browser).
- **`holistic/game-scout.jsx`** — the reference implementation of the model.
...
- **`seek` / `togglePlay` / `stepAB`** (shell) — the only state transitions; any click-seek drops to Review (pause).
- Feed auto-scroll via `scrollTop` off `offsetTop` (container `position: relative`); **never `scrollIntoView`**.

## Behaviour (v1)
- Open a final → **Review, paused, head at the first at-bat** (reconcile with PR 12 on in-app return).
- Single control = **action icon (▶/⏸) + mode tag (Review/Play)**, docked under the feed.
...
- **Past/future boundary** on both the feed and the scorebook row: played solid, current accented,
  future faded (~0.55) with no score chips.
