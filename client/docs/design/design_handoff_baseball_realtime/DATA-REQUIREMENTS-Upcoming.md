...
5. **Recommended split:** confirm or adjust the proposed **PR 9.5a** (MLB-data tier: groups 1, 2, 5, 6) vs **PR 9.5b** (Statcast tier: groups 3, 4 — joint with PR 6.5).

> **Numbering note:** the data-wiring follow-ups to PR 9 (the design port) are **PR 9.5a / PR 9.5b**, matching this project's convention (whole number = design port; `.5` = the data-gated wiring follow-up, as with PR 3 → 3.5 and PR 6 → 6.5). **PR 9.5a is DONE & APPROVED (Jun 6, 2026).** PR 9.5b is the remaining Statcast tier.
