...
Chunks below, each a separate PR. Stop and review at each boundary. (PR 3.5 was split out of PR 3 on June 1; **PR 8 — the Jun 3 polish pass — is appended at the end, pending review.**)
...
### PR 8 — Jun 3 polish pass (PENDING REVIEW — not yet signed off)

A batch of refinements from the Jun 3 review. The **Headshot fix is a bug fix and can land immediately**; the rest is **built but pending design-owner sign-off** — port for visual review (same posture as PR 6/7), don't treat as final.
...
**Acceptance:** Headshots never clip the chin anywhere. Landing title reflects the selected date and the Today button works; live cards show the inning; extra-inning finals show "FINAL (N)". A scheduled game renders the filled pregame layout (no blank "waiting" panels except the pitch-by-pitch empty state). The game return reads "← Back to games" and preserves the date. **Then: design-owner review → sign-off or change requests.**
