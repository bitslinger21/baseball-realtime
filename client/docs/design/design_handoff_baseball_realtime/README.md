...
4. **Two-column row** (`600px / 1fr`, `align-items: start`) — above the fold:
   ...
   - **Right — `PitchByPitchV2`** — fixed-height (640px) card with **INTERNAL scroll**:
     ...
     - **Scoring chip:** a PA that scored runs shows a soft-rust pill to the right of its outcome — "N run(s) score · [resulting score]" (e.g. "2 runs score · HOU 8 – 5 CHC"). Runs count in `T.accent`, score in mono. Non-scoring PAs have no chip.
