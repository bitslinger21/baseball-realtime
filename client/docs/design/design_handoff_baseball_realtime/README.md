...
1. **AppHeader** — right side has a **"Lineup ▾"** button (opens a lineup drawer — drawer itself NOT yet built), bell-with-badge, and "← Today's games".
...
4. **Two-column row** (`600px / 1fr`, `align-items: start`) — above the fold:
   - **Left — `MatchupLeft`** (sticky, `top:16`):
     - **Light play-state eyebrow** (`T.surfaceAlt`): inning glyph (▼ 9th) + `Bases` diamond + B/S/O `Pips` (balls navy, strikes ink, outs rust) on the left; `LivePill` on the right. *This is the only place the count lives now.*
   ...
   - **Right — `PitchByPitchV2`** — fixed-height (640px) card with **INTERNAL scroll**:
     - Header: "Pitch by pitch · N at-bats" + filter `Segmented` (All / Outcomes / Runs / K / HR / BB).
