...
| `ScorebookCell` | `inn, code, kind:'hit'/'out'/'walk'/'hbp', reached:0–4, scored, live, width` | One plate appearance as a scorebook diamond. Ink-on-cream: empty diamond = no base reached; a traced basepath + filled corner = bases reached; **dashed** = walk; `reached:4`/`scored` shades the diamond **green** (run). `live` = in-progress PA, neutral dashed frame (NOT rust). Numbers (`inn`, `code`) are mono. Used in the game-view batter-card "At-bats" row. |
...
4. **Two-column row** (`600px / 1fr`, `align-items: start`) — above the fold. The **entire left column is one sticky wrapper** (`top:16`) holding TWO stacked cards (`MatchupLeft` + `MatchupContext`) so they travel together; the wrapper height fills out to roughly match the 640px pitch-by-pitch frame on the right.
   - **Left card A — `MatchupLeft`:**
     ...
     - **Zone + batter** in a `280px / 1fr` grid: `StrikeZone` (240px) with all pitches of the current AB plotted as numbered dots + outcome legend; batter card (headshot, name, pos/B-T, slash line, "Today" line, **"At-bats" scorebook row**, "vs [pitcher]" career line).
       - **"At-bats" scorebook row** (added Jun 7, 2026): the **"Today" line shows only the summary** (`1-for-4`) — the per-AB results moved into diamonds below it. A small uppercase **"At-bats"** label sits over a **horizontally-scrolling row of `ScorebookCell`s** (`width:44` here), one per plate appearance today (inning label · diamond tracing bases reached · result code), oldest→newest left→right; the **newest/live PA is the last cell** (`live` — neutral dashed, NOT rust). At the design width ~5 cells fit; more scroll. **CRITICAL CSS:** the batter-card column, the inner single-column grid (`minmax(0,1fr)`, NOT `1fr`), the scorebook wrapper, and the scroll row ALL need `min-width:0`, or the row's intrinsic width inflates the `1fr` column and the batter card overlaps the PitchByPitch column. **Data:** per-PA `{ inning, resultCode, kind:hit/out/walk, basesReached }` — derivable from the same play-by-play feed that drives PitchByPitchV2; the run-shade (`scored`) and richer baserunning are a later enhancement (future.md F-003).
...
**Component:** `GameScreenV2Pregame` in `game-v2.jsx` (artboard `game-v2-pregame`). **Status: SIGNED OFF — simplified to 2 cards (Aug 1, 2026).**

An earlier pass explored a richer 7-card pregame layout (populated context card, starters
comparison, odds split, season series) but this was deliberately reduced to a lean 2-card
version, largely due to pregame data availability — most of that context (win-probability
split, head-to-head series, "top of the order") isn't reliably known this far ahead of first
pitch. The shipped design:
...
- **Matchup card:** a `Segmented` control (Preview / Head-to-head) in the page title toggles the whole body between:
  - **Preview** — a single centered empty state ("Game hasn't started" + explanatory copy) with the **"Lineups ▾"** button in the card header (opens the same `LineupsTray` used on the live view — bench/bullpen are available pregame even though the batting order isn't posted).
  - **Head-to-head** — renders `window.HeadToHeadScreen` (shared with the standalone head-to-head view) fed the same `LINEUPS`/`PROBABLES` mocks.

The richer per-section treatment (populated context card, starters comparison, odds, series) is
retired — do not re-port it.
...
- 3-column grid:
  ...
  - **Damage by location** card — a flex row: `StrikeZone` in **heat mode** (SLG by location, 150px, via the `HotZone` wrapper) on the left; on the right a compact panel — an **SLG color scale** (cold→hot gradient, `.000`→`.840+`) and **Hottest / Coldest** callouts (derived from the zone array: max + min cell, value in mono + zone name like "middle-middle" / "down & in"). A 62%-outside-the-zone insight line sits full-width below. **(BUG-004 fix, Jun 7 2026:** the "outside the zone" % is NOT hardcoded — it's derived in `PitchingTab` from a single `zonePct = { LHP, RHP }` source, blended by PA volume (`zonePA`) into `inZone`, with `outsideZone = 100 − inZone`. The same `zonePct` feeds the By-pitcher-handedness Zone% cells, so caption and table can't drift. With the current mock it resolves to **52%**, not the old static 62%.) **(BUG-005 fix, Jun 7 2026:** the caption's coldest-zone callout is NOT hand-written prose — it names `zoneNames[coldIdx]` + that cell's SLG directly, from the same hoisted zone arrays the heat map and the "Coldest" callout use. Don't reintroduce a hardcoded direction like "low/away"; derive it so caption and callout always agree. Confirm the heat-map in/away column orientation matches the intended batter's-view vs catcher's-view convention.)
- Below, 2-column grid:
