...
- **Sign-off status (as of Jun 3, 2026):** Landing ✅ · Game view v2 ✅ · Player **Overview ✅, Stats ✅, Splits ✅, Pitching ✅, History ✅** (all formally signed off; Pitching is batter-scoped). **Pending review (Jun 3 polish pass) — built but NOT yet signed off:** the **Pregame game-view state** (§2b), and the landing changes — **date-aware title**, **live-inning indicator**, **"FINAL (11)" extras**, and the **"← Back to games"** return relabel. The **`Headshot` chin-clip fix** (taller ratios + overlay stripe) is an atom-level **bug fix** and is applied everywhere. Mobile breakpoints, empty/loading/error states, postgame game-view state, and the Compare side-by-side view are not designed (Compare ships as a fake-door — see §3 Tab hero).
