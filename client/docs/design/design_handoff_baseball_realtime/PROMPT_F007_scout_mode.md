# Build F-007 — Game-view Review mode (finals): one play head, Play ↔ Review
...
> **No new API. No new data.** Review mode runs entirely on the play-by-play history the game view already consumes (the same `hydrate` replay the live feed uses). It is a pure client feature. If you find yourself adding an endpoint or a wire field, stop — that's the wrong change.

> **Naming (UI labels).** The single control's tag reads **Play** while playing and **Review** while paused — that's the user-facing vocabulary. (Internal identifiers may stay `scout*`; only the labels matter.) Lowercase "replay timer / interval" below means the *existing auto-advance mechanism* — keep that.

> Runnable reference in this package: `Game Scout Mode.html` (+ `holistic/game-scout.jsx`, `holistic/shared.jsx`, `holistic/game-v2.jsx`). Open it and drive it — press **▶ Play / Review**, click a feed at-bat, click a scorebook diamond, use ⏮/⏭. It is built on the real game-v2 layout, so what you see is the target composition.
...
- **Play** = *playing* — pitches auto-advance on the feed's existing replay timing.
- **Review** = *paused* — the head is frozen; the user analyzes.

**Pause** freezes at the current head (→ Review). **Play** resumes from that same head (→ Play). **Nothing rewinds on pause/play.**

**Scope: finals only (v1).** Live games keep their PR 11 follow / Jump-to-live behavior untouched; Review does **not** apply to live in v1.
...
- **persistent tag = the current mode**: "Review" when paused, "Play" when playing.

A bare `▶/⏸` loses the word "Review"; a bare segmented "Play/Review" loses the motion affordance — keep both in one control. (The prototype also shows a passive `PLAY/REVIEW` status echo in the band; optional.)
...
Optional convenience controls beside it: **⏮ / ⏭ step by one at-bat** (seek to the prev/next AB's last moment). These are **per-at-bat**, not per-inning. (Play speed — inherit the existing single replay interval for v1; 1×/2×/4× is optional, see "open choices".)

---

## Navigation — every path seeks the one head

- **Play** → sequential auto-advance, pitch by pitch, on the existing replay interval. At the end, stop (stay in Review at the final moment).
...
- **Clicking a past AB while in Play → seek AND pause** (enter Review), so what you clicked doesn't run away. Clicking **while in Review → seek, stay in Review.** (Cleanest rule: any click-seek sets `playing = false`.)
...
- **Default entry: opening a final → Review, paused, head at the START of the game** (first moment). Selecting a different final re-enters Review-at-start.
- **Reconcile with PR 12 position-persistence:** a *freshly selected* final opens Review-at-start; an in-app **RETURN** to a final you were just viewing should **restore** the saved position (head + expanded PA) per PR 12, in Scout (paused). A hard refresh falls back to Scout-at-start. (PR 12 is session-scoped; keep that.)

---

## ⚠️ CRITICAL port lessons (from PR 11 / PR 12 — do not relearn these the hard way)

1. **The replay drip-feed gate must run only while actively playing.** The single worst defect in the position-model ports was an incremental-reveal timer (history revealed one AB at a time on the replay clock) running in the **default** render path, so the screen visibly "played back from inning 1". For Review/paused and for any non-playing state, **render the full history in one paint.** Scope the incremental reveal strictly to active playback.
...
3. **Finals only.** Don't fabricate a "live" pill or follow behavior here; the LIVE pill is gated on `isLive` (BUG-008). Review is a finals state.
...
- **No win-probability or leverage work** (that's PR 3.5, gated on new backend fields). The Review prototype reuses the existing WinProb/Leverage cards as-is below the fold — leave them as they are.
...
- Play speed: inherit the single existing interval (v1 default) vs expose 1×/2×/4×.
...
## Acceptance
- Opening a **final** lands in **Review, paused, at the first at-bat** (or PR-12-restored position on in-app return).
- The control shows `▶ Review` when paused / `⏸ Play` when playing, and sits **docked under the feed**.
...
Open one PR titled **"F-007 — Game view: Review mode (finals) — one play head, Play ↔ Review"**; in the description, call out the active-playback-only drip-feed gate, the no-`scrollIntoView` auto-scroll, and the PR-12 entry reconciliation. Unpark F-007 from `future.md` when it lands.
