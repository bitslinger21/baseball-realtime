...
export class PollerService {
  ...
  private readonly winProbCache = new Map<
    string,
    {
      data: Array<{
        atBatIndex: number;
  ...
  private async fetchWinProb(gameId: string): Promise<
    Array<{
      atBatIndex: number;
      homeTeamWinProbability: number;
      leverageIndex?: number;
  ...
  > {
    ...
    if (
      cached != null &&
      Date.now() - cached.fetchedAt < this.WIN_PROB_TTL_MS
    ) {
      return cached.data;
    }
    try {
  ...
  public async fetchGameMeta(gameId: string): Promise<GameMeta> {
    ...
    const status: 'live' | 'scheduled' | 'final' | undefined = rawState
      ?.toLowerCase()
      .includes('final')
      ? 'final'
  ...
  /**
   * Same as fetchLatest, but also returns the history array fetchLatest already
   * computes internally and discards — avoids a second fetchHistory round trip
   * (getLiveFeed + winProbability) for callers that need both, e.g. the Baseball
   * IQ trigger/context builder.
   */
  public async fetchLatestWithHistory(
    gameId: string,
  ): Promise<{ latest: LiveUpdate; history: LiveUpdate[] }> {
    const history = await this.fetchHistory(gameId);
    if (history.length > 0) {
      return { latest: history[history.length - 1], history };
    }
    return { latest: await this.fetchLatest(gameId), history: [] };
  }
  ...
  public async fetchLatest(gameId: string): Promise<LiveUpdate> {
    ...
    const rhe: Linescore = {
      ...
      inningRuns: {
        away: lsInnings.map((inn: any) =>
          typeof inn?.away?.runs === 'number'
            ? (inn.away.runs as number)
            : null,
        ),
        home: lsInnings.map((inn: any) =>
          typeof inn?.home?.runs === 'number'
            ? (inn.home.runs as number)
            : null,
        ),
      },
    };
    ...
    const playSource: MlbPlay =
      frame?.play ?? (plays.currentPlay as MlbPlay | undefined) ?? {};
    ...
    const runners = Array.isArray(
      (currentPlay as unknown as { runners?: unknown }).runners,
    )
    ...
    for (const r of runners) {
      ...
      const end: string | null =
        typeof r?.movement?.end === 'string' ? r.movement.end : null;
      if (runnerId != null) {
    ...
    const hitKind: 'GB' | 'LD' | 'FB' | 'PU' | undefined =
      frame?.isFinalPitchOfAtBat === true
        ? (() => {
            const inPlayEvent = frame.play.playEvents?.find(
              (e) => e.details?.isInPlay === true,
            );
    ...
    const framePitch: PitchEvent | undefined = frame?.pitch as unknown as
      | PitchEvent
      | undefined;
  ...
  private getBrandingForAbbr(
    abbr: string | undefined,
  ): { primaryColor: string; logoUrl: string } | undefined {
  ...
  private computeScorebookCode(play: MlbPlay): string | undefined {
    ...
    if (v.includes('strikeout')) {
      ...
      const lastPitch = [...pitches]
        .reverse()
        .find((e) => e.isPitch === true || e.type === 'pitch');
    ...
    const creditSeq = (credits: MlbRunnerCredit[]): string[] => {
      const assists = credits
        .filter((c) => c.credit === 'f_assist')
        .map((c) => c.position?.code ?? '')
        .filter(Boolean);
      const putouts = credits
        .filter((c) => c.credit === 'f_putout')
        .map((c) => c.position?.code ?? '')
        .filter(Boolean);
  ...
  private buildPitchFrames(
    ...
    for (const p of allPlays) {
      ...
      for (let i = 0; i < pitchEvents.length; i += 1) {
        ...
        const isFinalPitchOfAtBat =
          i === pitchEvents.length - 1 && about.isComplete === true;
        const outs =
  ...
  private mapFrameToLiveUpdate(
    ...
    const runners = Array.isArray(
      (frame.play as unknown as { runners?: unknown }).runners,
    )
    ...
    const batterPlayer =
      readPlayer(boxHome, batterIdNum) ?? readPlayer(boxAway, batterIdNum);
    const pitcherPlayer =
      readPlayer(boxHome, pitcherIdNum) ?? readPlayer(boxAway, pitcherIdNum);
    ...
    const lsTeams = linescore?.teams ?? {};
    const lsInnings2: any[] = Array.isArray(linescore?.innings)
      ? linescore.innings
      : [];
    const rhe: Linescore = {
      away: {
      ...
      inningRuns: {
        away: lsInnings2.map((inn: any) =>
          typeof inn?.away?.runs === 'number'
            ? (inn.away.runs as number)
            : null,
        ),
        home: lsInnings2.map((inn: any) =>
          typeof inn?.home?.runs === 'number'
            ? (inn.home.runs as number)
            : null,
        ),
  ...
  public async fetchHistory(gameId: string): Promise<LiveUpdate[]> {
    ...
    return frames.map((frame) => {
      const wp =
        frame.atBatIndex != null
          ? winProbByAtBat.get(frame.atBatIndex)
          : undefined;
